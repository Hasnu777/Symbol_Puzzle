...
class Puzzle():
	...
	def SavePuzzle(self):
		...
		with open(os.path.join(os.path.dirname(__file__), f"{filename}.txt"), 'w') as f:
			...
			for i, P in enumerate(self.__AllowedPatterns):
				f.write(f"{self.__AllowedSymbols[i]},{P.GetPatternSequence()}\n")
			f.write(f"{self.__GridSize}\n")
			for i in range(len(self.__Grid)):
				if self.__Grid[i].GetSymbol() != "-":
					f.write(f"{self.__Grid[i].GetSymbol()},{','.join(self.__Grid[i].GetSymbolsNotAllowed())}\n")
				else:
					f.write(f",{','.join(self.__Grid[i].GetSymbolsNotAllowed())}\n")
	...
	def AttemptPuzzle(self):
		...
		while not Finished:
			...
			# START CHANGE
			Save = input("Save the puzzle at its current state? (Y/N) ").upper() == "Y"
			if Save:
				self.SavePuzzle()
				return self.__Score
			# END CHANGE
