...
class Puzzle():  # Used to create puzzles from files stored with the program, or makes a default puzzle
    def __init__(self, *args):  # Arguments are all packed, so variable amount can be given. Acts as a tuple(?)
        if len(args) == 1:  # This is true when a file name is given. Gives default
            ...
            self.__GridSize = args[
                0]  # Takes in length of each side of the board (square board so row/column are equal)
            self.__Grid = []
            ...
            with open(Filename) as f:
                ...
                for Count in range(1, NoOfSymbols + 1):  # reads from line 1 I guess?
                    self.__AllowedSymbols.append(
                        f.readline().rstrip())  # TODO Hasnu: check how the file is read, and check formatting of the file
                NoOfPatterns = int(
                    f.readline().rstrip())  # Depends on file formatting. Within the try block so I'm guessing they're catching with error types?
                for Count in range(1, NoOfPatterns + 1):  # reading all patterns in the file, 1->Num+1 = 0->Num
                    Items = f.readline().rstrip().split(
                        ",")  # TODO Hasnu: check format of pattern in file to see why this happens
                ...
                self.__GridSize = int(
                    f.readline().rstrip())  # from acceptable file format, should raise error if no worky
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:  # not False equates to True
            ...
            while not Valid:  # not False equates to True
                try:
                    Column = int(
                        input("Enter column number: "))  # takes number, no validity check on column range though
                    Valid = True
                except:
                    pass  # if invalid value put in e.g. character
            ...
            if CurrentCell.CheckSymbolAllowed(Symbol):  # method to check if the symbol can be used/put in the cell
                ...
                AmountToAddToScore = self.CheckforMatchWithPattern(Row,
                                                                   Column)  # Checking if a pattern has been created
    ...
    # Below subroutine searches for patterns within a 5x5 grid (symbol entered being the center) Patterns fit in a
    # 3x3 grid and 9 possible 3x3 grid positions in 5x5 grid, so 9 total checks. Each check goes on the following
    # path: top-left to top-right, to bottom-right, to bottom-left, to mid-left, to center
    def CheckforMatchWithPattern(self, Row, Column):
        for StartRow in range(Row + 2, Row - 1, -1):
            for StartColumn in range(Column - 2, Column + 1):
                try:  # 3x3 grid check
                    # TASK 2.1 START CHANGE
